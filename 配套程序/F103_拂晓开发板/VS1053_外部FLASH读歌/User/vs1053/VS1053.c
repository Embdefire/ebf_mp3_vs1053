...
//	return SPI_I2S_ReceiveData(VS_SPI);
	...
	WriteBuf[0] = writedat;
	SI_SPI_readdata(WriteBuf, ReadBuf, 1);
	return ReadBuf[0];
}
...
void VS_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(VS_GPIO_DREQ_CLK|VS_GPIO_RST_CLK|VS_GPIO_XDCS_CLK|VC1053_XCS_CLK|VC1053_SCLK_CLK|VC1053_MISO_CLK|VC1053_MOSI_CLK , ENABLE);
	/****模拟SPI引脚初始化****/

  GPIO_InitStructure.GPIO_Pin = VC1053_SCLK_Pin;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(VC1053_SCLK_GPIO_Port, &GPIO_InitStructure);
  
  GPIO_InitStructure.GPIO_Pin = VC1053_MISO_Pin;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(VC1053_MISO_GPIO_Port, &GPIO_InitStructure);
  
	GPIO_InitStructure.GPIO_Pin = VC1053_MOSI_Pin;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(VC1053_MOSI_GPIO_Port, &GPIO_InitStructure);
	
  GPIO_InitStructure.GPIO_Pin = VC1053_XCS_Pin;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(VC1053_XCS_GPIO_Port, &GPIO_InitStructure);  
  
  /* 片选使能 */
  VC1053_XCS_ENABLE();
	
	VC1053_SCLK_H;			      //时钟拉高，时钟拉高要干写入的事
