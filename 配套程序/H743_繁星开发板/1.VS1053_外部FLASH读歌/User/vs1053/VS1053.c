...
//  /* 等待发送缓冲区为空，TXE事件 */
//  while (__HAL_SPI_GET_FLAG( &VS_SpiHandle, SPI_FLAG_TXP ) == RESET)
//   {
//    if((SPITimeout--) == 0) return SPI_TIMEOUT_UserCallback(0);
//   }

//  /* 写入数据寄存器，把要写入的数据写入发送缓冲区 */
//  WRITE_REG(VS_SpiHandle.Instance->TXDR, byte);

//  SPITimeout = SPIT_FLAG_TIMEOUT;

//  /* 等待接收缓冲区非空，RXNE事件 */
//  while (__HAL_SPI_GET_FLAG( &VS_SpiHandle, SPI_FLAG_RXP ) == RESET)
//   {
//    if((SPITimeout--) == 0) return SPI_TIMEOUT_UserCallback(1);
//   }

//  /* 读取数据寄存器，获取接收缓冲区数据 */
//  return READ_REG(VS_SpiHandle.Instance->RXDR);

HAL_SPI_TransmitReceive(&VS_SpiHandle, &byte, &data, 1,SPITimeout);
return data;
...
{	
	__HAL_SPI_DISABLE(&VS_SpiHandle);   	
	VS_SpiHandle.Instance               = VS_SPI;
  VS_SpiHandle.Init.Mode              = SPI_MODE_MASTER;
  VS_SpiHandle.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_64;
...
{	
	...
	VS_SpiHandle.Instance               = VS_SPI;
  VS_SpiHandle.Init.Mode              = SPI_MODE_MASTER;
  VS_SpiHandle.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_16;
  VS_SpiHandle.Init.Direction         = SPI_DIRECTION_2LINES;
  VS_SpiHandle.Init.CLKPhase          = SPI_PHASE_2EDGE;
  VS_SpiHandle.Init.CLKPolarity       = SPI_POLARITY_HIGH;
  VS_SpiHandle.Init.CRCCalculation    = SPI_CRCCALCULATION_DISABLE;
